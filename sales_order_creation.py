...
from database import get_all_customers_from_db, get_all_products_from_db, create_sales_order

class SalesOrderCreationWindow(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Create New Sales Order")
        ...
        save_button = ttk.Button(bottom_frame, text="Save Sales Order", command=self.save_sales_order)
    ...
    def save_sales_order(self):
        ...
        if not customer_selection or not self.items:
        ...
        try:
            create_sales_order(customer['id'], self.items)
            messagebox.showinfo("Success", "Sales Order created successfully!")
            self.destroy()
        except Exception as e:
            messagebox.showerror("Error", f"Error creating sales order: {e}")

if __name__ == '__main__':
    root = tk.Tk()
    app = SalesOrderCreationWindow(root)
    root.withdraw()
    app.mainloop()
