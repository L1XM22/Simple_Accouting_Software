...
from database import get_all_customers_from_db, get_all_products_from_db, create_quotation

class QuotationCreationWindow(tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Create New Quotation")
        ...
        save_button = ttk.Button(bottom_frame, text="Save Quotation", command=self.save_quotation)
    ...
    def save_quotation(self):
        ...
        try:
            create_quotation(customer['id'], self.items)
            messagebox.showinfo("Success", "Quotation created successfully!")
            self.destroy()
        except Exception as e:
            messagebox.showerror("Error", f"Error creating quotation: {e}")

if __name__ == '__main__':
    root = tk.Tk()
    app = QuotationCreationWindow(root)
    root.withdraw()
    app.mainloop()
